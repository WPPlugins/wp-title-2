...
function wptitle2_initial(){
	...
	if (!get_option('wp_title_2')){
		...
		add_option('wp_title_2',serialize($default));
	}
}

// Get Options
$wptitle2_options = unserialize(get_option('wp_title_2'));
...
function wptitle2_admin_init(){
	...
	if ($_POST['action']=='update_wp_title_2'){
		...
		if (!update_option('wp_title_2',serialize($wptitle2_options)))
			add_option('wp_title_2',serialize($wptitle2_options));
...
if ($wptitle2_options['posts']=='enabled'){ // if enabled from options, add input to Post edit form as well
...
<?php if (strpos($_SERVER['REQUEST_URI'],'page-new.php')>0 || strpos($_SERVER['REQUEST_URI'],'page.php')>0 || $wptitle2_options['posts']=='enabled') : ?>
		...
		<p>
			...
			<label for="wp_title_posts">
				...
				<span class="description">
					(<?php _e('Check this in order to enable WP Title for Posts','wp_title_2') ?>)
				</span>
			</label>
		</p>
		<p>
